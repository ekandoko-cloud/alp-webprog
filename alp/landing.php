...
$sqlBest = "
    ...
    FROM spare_parts sp
    LEFT JOIN sales_order_details sod ON sp.spareparts_id = sod.spareparts_id
    GROUP BY sp.spareparts_id, sp.name, sp.sku, sp.selling_price, sp.stock_qty, sp.image_url
    ORDER BY COUNT(sod.salesorderdetails_id) DESC
    LIMIT 4
";
    ...
    <section class="max-w-6xl mx-auto px-6 py-16 bg-gray-50 rounded-xl mb-16">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <?php if (empty($new_arrivals)): ?>
                <div class="col-span-4 text-center py-10 text-gray-500">Belum ada data produk terbaru.</div>
            <?php else: ?>
                <?php foreach ($new_arrivals as $item): ?>
                    <div class="relative bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md hover:border-[#1e3a5f] transition-all flex flex-col">
                        ...
                        <div class="h-48 bg-gray-100 flex items-center justify-center border-b border-gray-200 relative">
                            ...
                            <?php else: ?>
                                <span class="material-symbols-outlined text-gray-300 text-[64px]">inventory_2</span>
                            <?php endif; ?>
                        </div>
