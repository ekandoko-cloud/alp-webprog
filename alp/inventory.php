...
// 3. Restock (Create Purchase Order)
    if ($post_action == 'restock_part') {
        ...
        header("Location: inventory.php?tab=inbound");
        exit;
    }
...
<div class="flex flex-grow">
    ...
    <main class="flex-grow p-8">
        ...
        <div class="flex space-x-6 border-b border-gray-200 mb-6">
            <a href="?tab=parts"
               class="pb-3 text-sm font-bold <?= $tab == 'parts' ? 'border-b-2 border-[#1e3a5f] text-[#1e3a5f]' : 'text-gray-500 hover:text-gray-700' ?>">Data
                Suku Cadang (CRUD)</a>
            <a href="?tab=suppliers"
               class="pb-3 text-sm font-bold <?= $tab == 'suppliers' ? 'border-b-2 border-[#1e3a5f] text-[#1e3a5f]' : 'text-gray-500 hover:text-gray-700' ?>">Data
                Pemasok (CRUD)</a>
            <a href="?tab=inbound"
               class="pb-3 text-sm font-bold <?= $tab == 'inbound' ? 'border-b-2 border-[#1e3a5f] text-[#1e3a5f]' : 'text-gray-500 hover:text-gray-700' ?>">Penerimaan
                Barang</a>
        </div>
        ...
        <?php if ($tab == 'parts'): ?>
            <?php if ($view_action == 'add_part' || $view_action == 'edit_part'):
                ...
                <div class="bg-white p-6 border rounded-lg shadow-sm mb-6 max-w-3xl">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <?php if ($edit_data): ?><input type="hidden" name="spareparts_id"
                                                        value="<?= $edit_data['spareparts_id'] ?>"><?php endif; ?>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-3">
                            ...
                            <div>
                                ...
                                <input type="text" name="sku" value="<?= htmlspecialchars($edit_data['sku'] ?? '') ?>"
                                       class="w-full border p-2 rounded bg-gray-50" required>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-3">
                            <div>
                                ...
                                <select name="category_id" class="w-full border p-2 rounded bg-gray-50" required>
                                    <option value="" disabled <?= !$edit_data ? 'selected' : '' ?>>Pilih Kategori
                                    </option>
                            ...
                            <div>
                                ...
                                <input type="text" name="unit" value="<?= htmlspecialchars($edit_data['unit'] ?? '') ?>"
                                       placeholder="Pcs, Kg, Roll, dll" class="w-full border p-2 rounded bg-gray-50"
                                       required>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-4 mb-3">
                            <div>
                                ...
                                <input type="number" name="stock_qty" value="<?= $edit_data['stock_qty'] ?? '' ?>"
                                       class="w-full border p-2 rounded bg-gray-50" required>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-600 mb-1">Stok Minimum</label>
                                <input type="number" name="min_stock" value="<?= $edit_data['min_stock'] ?? '' ?>"
                                       class="w-full border p-2 rounded bg-gray-50" required>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-600 mb-1">Harga Jual (Rp)</label>
                                <input type="number" name="selling_price"
                                       value="<?= $edit_data['selling_price'] ?? '' ?>"
                                       class="w-full border p-2 rounded bg-gray-50" required>
                            </div>
                        </div>

                        <label class="block text-xs font-bold text-gray-600 mb-1 mt-2">Upload Gambar (Kosongkan jika
                            tidak ingin mengubah)</label>
                        <input type="file" name="image_file" accept="image/*"
                               class="w-full border p-2 mb-3 rounded bg-gray-50 text-sm">

                        <label class="block text-xs font-bold text-gray-600 mb-1">Deskripsi</label>
                        <textarea name="description" class="w-full border p-2 mb-4 rounded bg-gray-50"
                                  rows="2"><?= htmlspecialchars($edit_data['description'] ?? '') ?></textarea>

                        <div class="flex gap-3">
                            <button type="submit" class="px-4 py-2 bg-[#1e3a5f] text-white rounded">Simpan Data
                                (Create/Update)
                            </button>
            ...
            <?php elseif ($view_action == 'restock' && $view_id > 0):
                ...
                <div class="bg-white p-6 border rounded-lg shadow-sm mb-6 max-w-2xl border-blue-300">
                    ...
                    <p class="text-sm text-gray-500 mb-4 font-medium">Item: <?= htmlspecialchars($part_data['name']) ?>
                        (SKU: <?= htmlspecialchars($part_data['sku']) ?>)</p>
                    ...
                    <form method="POST">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-3">
                            <div>
                                ...
                                <input type="number" name="qty_ordered" min="1"
                                       class="w-full border p-2 rounded bg-gray-50" required>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-600 mb-1">Harga Satuan (Rp)</label>
                                <input type="number" name="unit_price" value="<?= $part_data['selling_price'] ?>"
                                       class="w-full border p-2 rounded bg-gray-50" required>
                            </div>
                        </div>

                        <label class="block text-xs font-bold text-gray-600 mb-1">Catatan (Opsional)</label>
                        <textarea name="notes" class="w-full border p-2 mb-5 rounded bg-gray-50" rows="2"
                                  placeholder="Contoh: Restock urgent"></textarea>

                        <div class="flex gap-3">
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded font-bold">Buat
            ...
            <?php else:
                ...
                <div class="flex justify-between mb-4">
                    ...
                    <a href="?tab=parts&action=add_part"
                       class="bg-[#1e3a5f] text-white px-4 py-2 rounded text-sm shadow-sm">+ Tambah Barang (Create)</a>
                </div>
                        ...
                        <?php while ($row = $parts->fetch_assoc()): ?>
                            <tr class="hover:bg-slate-50">
                                <td class="p-4">
                                    ...
                                    <?php else: ?>
                                        <div class="w-12 h-12 bg-gray-100 border text-gray-400 flex items-center justify-center rounded text-[10px]">
                                            No Img
                                        </div>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="p-4 font-medium text-slate-700">
                                    Rp <?= number_format($row['selling_price'], 0, ',', '.') ?></td>
                                <td class="p-4">
                                    <div class="flex items-center gap-3">
                                        <a href="?tab=parts&action=edit_part&id=<?= $row['spareparts_id'] ?>"
                                           class="text-yellow-600 hover:text-yellow-800 font-medium text-xs flex items-center gap-1"><span
                                                    class="material-symbols-outlined text-[16px]">edit</span> Update
                                            (Edit)</a>
                                        <a href="?tab=parts&action=restock&id=<?= $row['spareparts_id'] ?>"
                                           class="text-blue-600 hover:text-blue-800 font-medium text-xs flex items-center gap-1"><span
                                                    class="material-symbols-outlined text-[16px]">shopping_cart</span>
                                            Restock</a>
                                        <a href="?delete_part=<?= $row['spareparts_id'] ?>"
                                           onclick="return confirm('Hapus data suku cadang ini?')"
                                           class="text-red-600 hover:text-red-800 font-medium text-xs flex items-center gap-1"><span
                                                    class="material-symbols-outlined text-[16px]">delete</span> Delete
                                            (Hapus)</a>
                                    </div>
                                </td>
                            </tr>
        ...
        <?php if ($tab == 'suppliers'): ?>
            <?php if ($view_action == 'add_supplier' || $view_action == 'edit_supplier'):
                ...
                <div class="bg-white p-6 border rounded-lg shadow-sm mb-6 max-w-2xl">
                    ...
                    <form method="POST">
                        ...
                        <?php if ($supp_data): ?><input type="hidden" name="suppliers_id"
                                                        value="<?= $supp_data['suppliers_id'] ?>"><?php endif; ?>

                        <label class="block text-xs font-bold text-gray-600 mb-1">Nama Pemasok (Supplier)</label>
                        <input type="text" name="name" value="<?= htmlspecialchars($supp_data['name'] ?? '') ?>"
                               class="w-full border p-2 mb-3 rounded bg-gray-50" required>

                        <label class="block text-xs font-bold text-gray-600 mb-1">Kontak Person</label>
                        <input type="text" name="contact_person"
                               value="<?= htmlspecialchars($supp_data['contact_person'] ?? '') ?>"
                               class="w-full border p-2 mb-3 rounded bg-gray-50" required>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-3">
                            <div>
                                ...
                                <input type="text" name="phone"
                                       value="<?= htmlspecialchars($supp_data['phone'] ?? '') ?>"
                                       class="w-full border p-2 rounded bg-gray-50" required>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-600 mb-1">Email</label>
                                <input type="email" name="email"
                                       value="<?= htmlspecialchars($supp_data['email'] ?? '') ?>"
                                       class="w-full border p-2 rounded bg-gray-50" required>
                            </div>
                        </div>

                        <label class="block text-xs font-bold text-gray-600 mb-1">Alamat</label>
                        <textarea name="address" class="w-full border p-2 mb-5 rounded bg-gray-50" rows="3"
                                  required><?= htmlspecialchars($supp_data['address'] ?? '') ?></textarea>

                        <div class="flex gap-3">
                            <button type="submit" class="px-4 py-2 bg-[#1e3a5f] text-white rounded">Simpan Data Pemasok
                                (Create/Update)
                            </button>
            ...
            <?php else: ?>
                <div class="flex justify-between mb-4">
                    ...
                    <a href="?tab=suppliers&action=add_supplier"
                       class="bg-[#1e3a5f] text-white px-4 py-2 rounded text-sm shadow-sm">+ Tambah Pemasok (Create)</a>
                </div>
                <div class="bg-white border rounded-lg overflow-hidden shadow-sm">
                        ...
                        while ($s = $suppliers_list->fetch_assoc()): ?>
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="p-4 text-xs text-gray-600 max-w-xs truncate"
                                    title="<?= htmlspecialchars($s['address']) ?>"><?= htmlspecialchars($s['address']) ?></td>
                                <td class="p-4 flex gap-3">
                                    <a href="?tab=suppliers&action=edit_supplier&id=<?= $s['suppliers_id'] ?>"
                                       class="text-yellow-600 hover:underline text-xs">Update (Edit)</a>
                                    <a href="?delete_supplier=<?= $s['suppliers_id'] ?>"
                                       onclick="return confirm('Hapus data pemasok ini?')"
                                       class="text-red-600 hover:underline text-xs">Delete (Hapus)</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                        </tbody>
        ...
        <?php if ($tab == 'inbound'): ?>
            <?php if ($view_action == 'receive' && $view_id > 0):
                ...
                <div class="bg-white p-6 border rounded-lg shadow-sm mb-6 max-w-md border-green-300">
                    ...
                    <form method="POST">
                        ...
                        <p class="text-sm font-bold text-gray-700 mb-1"><?= htmlspecialchars($od_data['part_name']) ?>
                            (PO-<?= $od_data['purchaseorders_id'] ?>)</p>
                        <p class="text-xs text-gray-500 mb-4">Sisa yang belum diterima: <span
                                    class="font-bold text-red-600"><?= $sisa_terima ?> <?= htmlspecialchars($od_data['unit']) ?></span>
                        </p>

                        <label class="block text-xs font-bold text-gray-600 mb-1">Jumlah Diterima</label>
                        <input type="number" name="qty_received" max="<?= $sisa_terima ?>" value="<?= $sisa_terima ?>"
                               min="1" class="w-full border p-3 mb-6 rounded bg-gray-50 text-lg font-bold" required>

                        <div class="flex gap-3">
                            <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded font-bold">Konfirmasi
                                Penerimaan (Update)
                            </button>
            ...
            <?php else:
                ...
                <div class="mb-4">
                    ...
                    <p class="text-sm text-slate-500">Klik "Terima" ketika barang dari supplier tiba di gudang untuk
                        memperbarui stok.</p>
                </div>
                <div class="bg-white border rounded-lg overflow-hidden shadow-sm">
                    <table class="w-full text-sm text-left">
                        ...
                        <?php if ($inbounds && $inbounds->num_rows > 0): ?>
                            <?php while ($ib = $inbounds->fetch_assoc()): ?>
                                <tr class="hover:bg-slate-50">
                                    <td class="p-4 text-xs font-mono"><?= date('d M Y', strtotime($ib['order_date'])) ?>
                                        <br>#PO-<?= $ib['purchaseorders_id'] ?></td>
                                    ...
                                    <td class="p-4">
                                        <a href="?tab=inbound&action=receive&id=<?= $ib['orderdetails_id'] ?>"
                                           class="inline-block bg-green-600 text-white px-3 py-1.5 rounded text-xs hover:bg-green-700 font-bold">Terima
                                            Barang</a>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="p-8 text-center text-gray-400">Semua pesanan sudah diterima atau
