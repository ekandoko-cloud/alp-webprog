...
<nav class="border-b border-gray-200 bg-white sticky top-0 z-50 h-14 flex items-center">
    <div class="max-w-6xl mx-auto px-6 flex items-center justify-between w-full relative">
        ...
        <div class="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center gap-8 text-sm text-gray-600 font-medium">
            <a href="products.php" class="text-[#1e3a5f] nav-active">Produk</a>
        ...
        <div class="flex-shrink-0 flex items-center justify-end gap-3">
            ...
            <a href="cartmenu.php" class="bg-[#1e3a5f] text-white text-sm font-medium px-4 py-1.5 rounded-md flex items-center gap-2 hover:bg-[#162d4a] transition">
                <span class="material-symbols-outlined text-[18px]">shopping_cart</span> Keranjang
            </a>
        </div>
    ...
    <aside class="w-full md:w-64 flex-shrink-0 pr-6">
        <form method="GET" class="flex flex-col gap-6">
            <div class="flex flex-col gap-2">
                ...
                <input name="search" class="w-full pl-3 py-2 bg-gray-50 border rounded-md text-sm" placeholder="Cari suku cadang..." type="text" value="<?= htmlspecialchars($_GET['search'] ?? '') ?>"/>
            </div>
            ...
            <button type="submit" class="bg-[#1e3a5f] text-white py-2 rounded text-sm font-bold">Terapkan Filter</button>
        </form>
    </aside>
            ...
            <?php while ($row = $result->fetch_assoc()):
                ...
                <article class="bg-white rounded-lg p-5 flex flex-col border border-gray-100 shadow-sm">
                    <div class="h-48 w-full bg-gray-50 rounded-md border mb-4 overflow-hidden relative">
                        <img class="w-full h-full object-cover" src="<?= htmlspecialchars($row['image_url'] ?? 'images/default.jpg') ?>"/>
                    ...
                    <div class="flex flex-col flex-1">
                        ...
                        <div class="mt-auto">
                            <p class="text-xl font-bold text-gray-900 mb-3">Rp <?= number_format($row['selling_price'], 0, ',', '.') ?></p>
                            <form action="cart.php" method="POST">
                                ...
                                <input type="number" name="qty" value="1" min="1" max="<?= $row['stock_qty'] ?>" class="w-full border p-2 mb-2 rounded text-sm">
                                <button type="submit" <?= !$is_available ? 'disabled' : '' ?> class="w-full bg-[#1e3a5f] disabled:bg-gray-400 text-white text-sm font-medium px-4 py-2 rounded-md transition">
                                    + Tambah ke Keranjang
                                </button>
                            </form>
                        </div>
